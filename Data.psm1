...
Function Get-SystemSpecifications()
{
    ...
    $OS = Get-OS;
    $Kernel = Get-Kernel;
    $Uptime = Format-Uptime-Screenfetch;
...
Function Format-Uptime-Screenfetch()
{
    $uptime_days = (Get-Uptime-Screenfetch | Select-Object -Expand Days)
    $uptime_hours = (Get-Uptime-Screenfetch | Select-Object -Expand Hours)
    $uptime_minutes = (Get-Uptime-Screenfetch | Select-Object -Expand Minutes)
    $uptime_seconds = (Get-Uptime-Screenfetch | Select-Object -Expand Seconds)
    $FormattedUptime = "${uptime_days}d ${uptime_hours}h ${uptime_minutes}m ${uptime_seconds}s "
    return $FormattedUptime;
}

Function Get-Uptime-Screenfetch()
{
    if (($PSVersionTable.PSVersion | Select-Object -Expand Major) -lt 6) {
        return ((get-date) - (gcim Win32_OperatingSystem).LastBootUpTime)
    }
    else {
        return Get-Uptime
    }
